package com.sulake.core.runtime.events
{
   ...
   public class EventDispatcher implements IEventDispatcher, IDisposable
   {
      ...
      private static const const_768:uint = 2;
       
      
      private var var_703:Error;
      ...
      public function dispatchEvent(param1:Event) : Boolean
      {
         if(!_disposed)
         {
            ...
            if(_result == const_768)
            {
               if(var_703 != null)
               {
                  Core.crash("Error catched in EventDispatcher: undefined",var_703.errorID,var_703);
               }
               else
               {
                  Core.crash("Error catched in EventDispatcher. No error data available!",0,var_703);
               }
            }
      ...
      private function eventProcessor(param1:Event) : void
      {
         ...
         var array:Array = var_264[event.type];
         if(array)
         {
            callbacks = [];
         ...
         _result = !!event.isDefaultPrevented() ? uint(const_1081) : uint(const_492);
      }
      ...
      public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            for(_loc1_ in var_264)
            {
               _loc2_ = var_264[_loc1_] as Array;
               for each(_loc3_ in _loc2_)
               {
                  _loc3_.callback = null;
               }
               delete var_264[_loc1_];
            }
            var_264 = null;
